...
class CreateUnusableSpaceView(QWidget):
    ...
    def __init__(self,store,appController,unusableSpaceController):
        ...
        label = QLabel("Créer zone inutilisable")   
        layout = QFormLayout()
        self.name = QLineEdit()
        ...
        validButton.clicked.connect(self.validUnusedSpace)
    ...
    def validUnusedSpace(self):
        self.unusableSpaceController.addUnusableSpace(self.selectedRoom,self.name.text(),self.lineLength.text(),self.lineWidth.text(),self.selectedOrientation)
